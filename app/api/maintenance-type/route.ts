...
const UPSTREAM_URL = "https://tenaris-468894656254.us-central1.run.app/api/data/maintenance-type";
  ...
  try {
    ...
    if (totalPages > 1) {
      ...
      const responses = await Promise.all(
        pendingPages.map(async (page) => {
          ...
          if (!response.ok) {
            ...
            throw new Error(text || `Error ${response.status} en page ${page}`);
          }
      ...
      for (const payload of responses) {
        const pageData = payload as { data?: unknown[] };
    ...
    return Response.json(
      {
        error: "No se pudo obtener maintenance-type",
